...
DhtSensorRead getDhtSensorData(){
  static DhtSensorRead lastRead;
...
//data from DHT sensor
  DhtSensorRead dhtData = getDhtSensorData(); 
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(",\"temperature\":"));
  (void)az_span_dtoa(temp_span,dhtData.temperature,2,&temp_span);
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(",\"humidity\":"));
  (void)az_span_dtoa(temp_span,dhtData.humidity,2,&temp_span);

//analog input from LDR
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(",\"light\":"));
  (void)az_span_u32toa(temp_span,getLightData(),&temp_span);
